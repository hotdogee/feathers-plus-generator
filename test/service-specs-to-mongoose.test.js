...
describe('Creates Mongoose Schemas', function () {
  it('includes enum attributes', function () {
    ...
    assert.deepEqual(
      mongooseSchema.roles[0].enum,
      ['admin'],
      'roles enum was correctly applied'
    )

    assert.deepEqual(
      mongooseSchema.roles1[0].enum,
      ['admin'],
      'roles1 enum was correctly applied'
    )

    assert.deepEqual(
      mongooseSchema.dept.enum,
      ['acct'],
      'dept enum was correctly applied'
    )
  })
  it('includes default attributes', function () {
    ...
    assert.deepEqual(
      mongooseSchema.status.default,
      'active',
      'status default value was correctly applied'
    )

    assert.deepEqual(
      mongooseSchema.genre.default,
      'action',
      'genre default value was correctly applied'
    )

    assert.deepEqual(
      mongooseSchema.registerdDate.default,
      Date.now,
      'registerdDate default value was correctly applied'
    )
  })
  it('properly generates model for array of objects', function () {
    const mongooseSchema = serviceSpecsToMongoose(usersSchema.schema)
    assert.deepEqual(
      mongooseSchema.categories,
      [
        {
          name: String,
