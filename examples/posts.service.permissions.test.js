...
function ro(serviceName, methods) {
  const rolesArray = methods
    .split('')
    .map(method => `${serviceName || SERVICE_NAME}::${serviceMethods[method]}`)
...
describe('Test posts/posts.service.permissions.test.js', () => {
  describe('Role Based Access Control', () => {
    ...
    decisionTable.forEach(
      ([user, isAuth, roles, provider, method, id, errCode]) => {
        const testDesc = `${provider ? provider : 'server'}, ${method}${
          method !== 'create' ? `, ${id}` : ''
        }, ${isAuth ? ' auth' : '!auth'}, ${roles}`
            ...
            switch (method) {
              ...
              case 'update':
                await app
                  ...
                  .update(ids[id], { body: 'bar' }, params)
                break
              case 'patch':
                await app
                  .service(SERVICE_NAME)
                  .patch(ids[id], { body: 'bar' }, params)
                break
              case 'remove':
