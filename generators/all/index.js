...
module.exports = class AllGenerator extends Generator {
  async prompting () {
    ...
    this.log(
      [
        ...
        chalk.green.bold(
          ` in ${specs.options.ts ? 'TypeScript' : 'JavaScript'}`
        )
      ].join('')
    )
    this.log()
  ...
  install () {
    ...
    this.log(
      chalk.green(
        `Installing ${dependenciesList.dev.length} development dependencies.`
      )
    )
    ...
    if (dependenciesList.dev.length) {
      this[method](dependenciesList.dev, {
        [packager === 'npm' ? 'saveDev' : 'dev']: true
      })
    }
    ...
    const removeFiles = currFiles
      .filter(path => path.substr(-3) === currSuffix)
