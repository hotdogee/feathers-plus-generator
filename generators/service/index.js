...
module.exports = class ServiceGenerator extends Generator {
  async prompting () {
    ...
    const ifCalledByAuthentication =
      specs._generators.indexOf('authentication') !== -1
    ...
    const prompts = [
              ...
              generator.log(
                chalk.green(
                  [
                    ...
                    `"services/${kebabCase(
                      input
                    )}/${input}.schema.js". Then (re)generate this service.`,
                    '',
      ...
      },
      {
        name: 'nameSingular',
        message (answers) {
          return `What would you call one row in the ${chalk.green(
            answers.name || props.name
          )} database?`
        },
        default (answers) {
          return (
            serviceSpecs.nameSingular || singular(answers.name || props.name)
          )
        },
        validate (input) {
          if (input.trim() === '') {
      ...
      },
      {
        ...
        default (answers) {
          defaultSubFolder =
            serviceSpecs.subFolder || answers.subFolder || props.subFolder
