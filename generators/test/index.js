...
module.exports = class CodelistGenerator extends Generator {
  async prompting () {
    ...
    const hookChoices = Object.keys(specs.hooks || {})
      .map(name => {
    ...
    const serviceChoices = Object.keys(specs.services)
      .sort()
      .map(name => {
    ...
    const prompts = [
      ...
      },
      {
        ...
        type: 'list',
        when: answers =>
          answers.testType === 'hookUnit' || answers.testType === 'hookInteg',
        choices: hookChoices
      },
      {
        name: 'serviceName',
        ...
        type: 'list',
        when: answers =>
          answers.testType === 'serviceUnit' ||
          answers.testType === 'serviceInteg',
    ...
    return this.prompt(prompts).then(answers => {
      if (
        ['serviceInteg', 'authBase', 'authServices'].indexOf(
          answers.testType
        ) !== -1
      ) {
        generator.log('\n')
      ...
      if (['authBase', 'authServices'].indexOf(answers.testType) !== -1) {
        const environmentsAllowingSeedData =
          specs.app.environmentsAllowingSeedData
        ...
        if (environmentsAllowingSeedData.length) {
          ...
          generator.log(
            [
              chalk.green.bold(
                'This test will not run until a test environment is set using '
              )
            ].join('')
          )
          generator.log(
            [chalk.yellow.bold('generate app'), chalk.green.bold('.')].join('')
          )
        }
        ...
        if (answers.testType === 'authServices') {
          generator.log(
            [
              ...
              chalk.yellow.bold("authenticate('local')") +
                ' hooks. You could even use the ' +
                chalk.yellow.bold('disallow'),
              'common hook to entirely disallow client access.',
              '',
              'Only such authentication or disallow changes need be specified in file ',
              chalk.yellow.bold('config/default.js') +
                ' as prop ' +
                chalk.yellow.bold('tests.client.overriddenAuth') +
                '.',
              '',
              'For example:',
