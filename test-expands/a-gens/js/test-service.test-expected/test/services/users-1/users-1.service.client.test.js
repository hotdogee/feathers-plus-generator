...
if (
  ...
  console.log('SKIPPED - Test users-1/users-1.service.client.test.js');
...
describe('Test users-1/users-1.service.client.test.js', () => {
  let app;
  let client;
  let server;
  ...
  before(async function() {
    ...
    const result = await app.service('/users-1').find({ query: { email } });
    if ((result.data || result).length === 0) {
      await app.service('/users-1').create({ email, password });
    }
  ...
  it('registered the service', () => {
    const service = client.service('/users-1');
  ...
  it('???', async () => {
    ...
    /*
    const record = await client.service('/users-1').create({
