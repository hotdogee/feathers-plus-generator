...
function validateSchemaExtensions (mapping, feathersSchemas) {
  ...
  Object.keys(feathersSchemas).forEach(schemaName => {
    ...
    if (feathersExtensions !== undefined) {
      if (
        typeof feathersExtensions !== 'object' ||
        feathersExtensions === null
      ) {
...
function validateGraphql (mapping, graphqlExt) {
  ...
  if (
    graphqlExt.sql !== undefined &&
    (typeof graphqlExt.sql !== 'object' || graphqlExt.sql === null)
  ) {
    errors.push(`.sql is typeof ${graphqlExt.sql} not Object. (204)`)
  }
  if (
    graphqlExt.discard !== undefined &&
    (typeof graphqlExt.discard !== 'object' || graphqlExt.discard === null)
  ) {
...
function validateGraphqlAdd (mapping, graphqlAdd) {
  ...
  if (typeof graphqlRelation.ourTable !== 'string') {
    errors.push(
      `.relation.ourTable is typeof ${
        graphqlRelation.ourTable
      } not String. (402)`
    )
  }
  if (typeof graphqlRelation.otherTable !== 'string') {
    errors.push(
      `.relation.otherTable is typeof ${
        graphqlRelation.otherTable
      } not String. (403)`
    )
  }
