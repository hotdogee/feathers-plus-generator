...
module.exports = function serviceSpecsToGraphql (feathersSchemas) {
  ...
  Object.keys(feathersSchemas).forEach(schemaName => {
    ...
    if (
      !feathersExtensions._ifGraphql ||
      !graphqlName ||
      (!graphqlExt.service && !graphqlExt.sql)
    ) {
      return
    }
    ...
    fieldSchemas.push(
      [
        `type ${graphqlName} {`,
        feathersSchemaToGraphqlSchema(
          feathersSchemas[schemaName],
          feathersSchemas[schemaName]._extensions
        ),
        Object.keys(add)
          .map(field => `  ${field}${add[field].args}: ${add[field].type}`)
          .join(EOL),
        '}',
        ' '
      ]
...
function feathersSchemaToGraphqlSchema (
  feathersSchema,
  feathersExtension,
  depth = 1
) {
  ...
  const graphqlTypes = Object.keys(properties).map(name => {
    ...
    if (type === 'object') {
      return `${name}: ${schemaTypeToGraphql.object}`
    }
    ...
    if (type === 'array') {
      ...
      if (typeof items === 'object') {
        ...
        throw new Error(
          `Property "${name}" is an array with an invalid "items".`
        )
      }
    }

    // Handle simple property
    return `${name}: ${array ? '[' : ''}${schemaTypeToGraphql[type.trim()]}${
      req ? '!' : ''
    }${array ? ']' : ''}`
  })
