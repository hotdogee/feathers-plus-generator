...
module.exports = class BaseGenerator extends Generator {
  constructor (args, opts) {
    ...
    if (!semver.satisfies(process.version, '>= 10.0.0')) {
      this.log.error(
        'The generator is only tested to work with Node v10.0.0 and up!'
      )
    }

    this._opts = opts
    this.generatorPkg = this.fs.readJSON(
  ...
  getNameSpace (subFolder = '', ifConvert) {
    ...
    const parts = subFolder
      .split('/')
      .map(part => (ifConvert ? makeFileName(part) : part))
    ...
    if (!specs) {
      this.log.error(
        chalk.green.bold('Dir ') +
          ...
          chalk.green.bold(
            ' does not contain an app created by this generator.'
          )
      )
  ...
  _packagerInstall (deps, options) {
    ...
    const existingDependencies =
      this.pkg[isDev ? 'devDependencies' : 'dependencies'] || {}
    const dependencies = deps
      .filter(current => !existingDependencies[current])
      ...
      .map(dependency => {
        ...
        if (!version) {
          this.log(
            `No locked version found for ${dependency}, installing latest.`
          )
  ...
  callWritingFromPrompting () {
    ...
    const ifWin10 =
      platform()
        .toLowerCase()
        .includes('win') && release().substr(0, 3) === '10.'
