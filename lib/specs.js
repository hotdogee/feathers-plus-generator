...
const {
  ...
  resetForTest: resetFragments
} = require('./code-fragments')
...
async function setPath (generator1) {
  if (!generator) {
    // single initialize in case of composeWith
    ...
    for (let i = 1; i <= 2; i++) {
      ...
      if (notifier && notifier.update && notifier.update.lastest) {
        console.log(
          `Updated generator version on npm is ${notifier.update.lastest}`
        )
      }

      notifier.notify({
        message:
          'Updated generator available.  \nRun ' +
    ...
    if (stashedSpecs.graphql) {
      stashedSpecs.graphql.doNotConfigure =
        stashedSpecs.graphql.doNotConfigure || false
    }
    ...
    // stashedSpecs contains 'current' config/default.js as multiple generators may change it
    ...
    stashedSpecs._isRunningTests =
      root.startsWith('/private/') || root.indexOf(tmpdir()) !== -1
...
function initSpecs (what, info) {
  ...
  if (specsOptions.inspectConflicts === undefined) {
    specsOptions.inspectConflicts = false
  }
  if (specsOptions.semicolons === undefined) specsOptions.semicolons = false
  ...
  specsApp.environmentsAllowingSeedData =
    specsApp.environmentsAllowingSeedData || ''
  specsApp.seedData = specsApp.seedData || true
  ...
  switch (what) {
    ...
    case 'authentication':
      ...
      stashedSpecs.authentication.strategies =
        stashedSpecs.authentication.strategies || []
      stashedSpecs.authentication.entity =
        stashedSpecs.authentication.entity || undefined
      break
    case 'graphql':
      ...
      graphql.sqlInterface =
        graphql.sqlInterface ||
        (graphql.strategy === 'sql' ? 'sequelize' : null)
...
function updateSpecs (what, props, whosCalling) {
  if (!generator) {
    throw new Error('specs#setPath not called before other funcs. (specs)')
  }
  let serviceSpecs,
    hookSpecs,
    connectionSpecs,
  ...
  switch (what) {
    ...
    case 'app':
      ...
      app.environmentsAllowingSeedData = props.environmentsAllowingSeedData
        .split(',')
        .map(str => str.trim())
        .filter(str => !!str)
        .join(',')
    ...
    case 'hook':
      ...
      break
    case 'connection':
      connectionSpecs = stashedSpecs.connections[
        props.adapter || props.database
      ] = {}
