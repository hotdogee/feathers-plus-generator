...
module.exports = function specsExpand (specs) {
  ...
  Object.keys(connections).forEach(adapter1 => {
    ...
    const { dependencies, connection } = getConnectionInfo(
      connectionString,
      database,
      adapter
    )
...
function getConnectionInfo (connectionString, database, adapter) {
  ...
  switch (database) {
      ...
      return {
        ...
        connection:
          connectionString.indexOf('://') === -1
            ? connectionString
            : connectionString.substring(7, connectionString.length)
      }
    ...
    case 'sqlite':
      ...
      return {
        ...
        connection: {
          ...
          connection:
            database === 'sqlite' && typeof connectionString === 'string'
              ? {
                // Hack: Have to distinguish between newly added connection and an already configured one.
                filename:
                    connectionString.indexOf('://') === -1
                      ? connectionString
                      : connectionString.substring(9, connectionString.length)
              }
              : connectionString
        }
      }
    ...
    default:
      throw new Error(
        `Invalid database '${database}'. Cannot assemble configuration.`
      )
  }
}
