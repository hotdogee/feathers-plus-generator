...
async function refreshCodeFragments (destinationRoot1) {
  ...
  try {
    ...
    console.log(
      'Source scan took %ds %dms',
      diffTime[0],
      Math.round(diffTime[1] / 1000000)
    )
    console.log('')
  } catch (err) {
    console.log(err)
...
async function extractProject () {
  try {
    const files = glob.sync(`${destinationRoot}/**/*.{js,ts}`, {
      ignore: [
    ...
    if (!doingCustomCode) {
      ...
      if (leaderLen && !anyTrailer) {
        ...
        if (code[filePath][codeLocation]) {
          console.log(
            `. Found replacement custom code for location '${codeLocation}' in ${filePathSrc.substr(
              destinationRoot.length + 1
            )}.`
          )
        }
  ...
  */
  ...
  return (name, defaultCode, forceAsNonDefault, options = {}) => {
    ...
    if (moduleCode && name in moduleCode) {
      if (!moduleCode[name].length) {
        // allow fragments with no lines
      ...
      return [`${genLeader} ${name}`]
        .concat(moduleCode[name], `${spaces}${genTrailer}`)
        .join(EOL)
    }
    ...
    if (defaultCode) {
      ...
      return [`${forceAsNonDefault ? genLeader : genDefaultLeader} ${name}`]
        .concat(defaultCode, `${spaces}${genTrailer}`)
        .join(EOL)
    }
...
function getFragment (filePath) {
  ...
  return name =>
    moduleCode && name.trim() in moduleCode
      ? moduleCode[name.trim()]
      : undefined
}
